...
@section('content')
    ...
    <div class="container-fluid note-has-grid">
        <ul class="nav nav-pills p-3 mb-3 rounded align-items-center card flex-row">
            ...
            <li class="nav-item">
                <a data-bs-toggle="tab" href="#acc" role="tab"
                    class="nav-link note-link d-flex align-items-center justify-content-center px-3 px-md-3 me-0 me-md-2 text-body-color "
                    id="note-social">
        ...
        <div class="tab-content">
                                ...
                                <div class="card-body">
                                    <div class="d-flex flex-wrap gap-2">
                                        <p class="badge bg-primary-subtle text-primary" style="font-size: 12px">
                                            Batas:
                                            {{ \Carbon\Carbon::parse($challenge->deadline)->locale('id_ID')->isoFormat('dddd, D MMMM YYYY') }}
                                        </p>
                                    ...
                                    <button type="button" class="btn btn-primary w-100 btn-add"
                                        data-id="{{ $challenge->id }}" data-challenge="{{ $challenge->title }}"
                    ...
                    @empty
                        <div class="col-md-12 text-center pt-5">
                            <img src="{{ asset('assets-user/dist/images/products/empty-shopping-bag.gif') }}" alt="No Data"
                                height="120px" />
                            <h6 class="text-center">Belum Ada Tantangan</h6>
                        </div>
                    @endforelse
                </div>
            </div>
            <div class="tab-pane" id="done" role="tabpanel">
                <div class="d-flex flex-wrap  all-category note-important">
                    @forelse ($challengePendings as $challengePending)
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="d-flex flex-wrap gap-2">
                                        <p class="badge bg-primary-subtle text-primary" style="font-size: 12px">
                                            Batas:
                                            {{ \Carbon\Carbon::parse($challengePending->deadline)->locale('id_ID')->isoFormat('dddd, D MMMM YYYY') }}
                                        </p>
                                        <p class="badge bg-info-subtle text-info" style="font-size: 12px">
                                            {{ $challengePending->challenge->level->label() }}
                                        </p>
                                    </div>
                                    <h4>{{ $challengePending->challenge->title }}</h4>
                                    <p class="text-mute">
                                ...
                                <div class="card-body">
                                    <div class="d-flex align-items-center gap-3">
                                        <h3
                                            ...
                                            {{ $challengeDone->score }}</h3>
                                        <h5>{{ $challengeDone->challenge->title }}</h5>
                                    </div>
                                    <p class="text-muted">{{ $challengeDone->challenge->description }}</p>
                                    <div class="d-flex gap-2 flex-wrap">
                                        <p class="badge bg-primary-subtle text-primary" style="font-size: 12px">Batas:
                                            {{ \Carbon\Carbon::parse($challengeDone->deadline)->locale('id_ID')->isoFormat('dddd, D MMMM YYYY') }}
                                        </p>
                                        <p class="badge bg-danger-subtle text-danger" style="font-size: 12px">
                                            {{ $challengeDone->challenge->level }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="modal fade" id="add" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
            ...
            <div class="modal-dialog modal-dialog-scrollable modal-lg">
                <div class="modal-content">
                    ...
                    <form action="{{ route('challenge_online.store') }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('POST')
                        <div class="modal-body">
                            <div class="mb-2">
                                <h6>
                                    <svg width="26" class="me-2" height="26" viewBox="0 0 26 26"
        ...
        <div class="modal-dialog modal-dialog-scrollable modal-lg">
            <div class="modal-content">
                ...
                <form method="post" enctype="multipart/form-data" id="form-update">
                    ...
                    <div class="modal-body">
                        <div class="mb-2">
                            <h6 class="d-flex mb-0 pb-0">
                                <svg width="26" class="me-2" height="26" viewBox="0 0 26 26" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M20.5837 2.1665C21.4126 2.16646 22.2103 2.48319 22.8134 3.05189C23.4166 3.6206 23.7796 4.39829 23.8282 5.22584L23.8337 5.4165V20.5832C23.8337 21.4121 23.517 22.2098 22.9483 22.813C22.3796 23.4161 21.6019 23.7791 20.7743 23.8278L20.5837 23.8332H5.41699C4.58801 23.8332 3.79035 23.5165 3.18721 22.9478C2.58407 22.3791 2.22104 21.6014 2.17241 20.7738L2.16699 20.5832V5.4165C2.16695 4.58753 2.48368 3.78986 3.05238 3.18672C3.62109 2.58358 4.39877 2.22055 5.22633 2.17192L5.41699 2.1665H20.5837ZM13.0003 11.9165H11.917L11.7902 11.9241C11.5269 11.9554 11.2843 12.0822 11.1082 12.2805C10.9321 12.4787 10.8349 12.7347 10.8349 12.9998C10.8349 13.265 10.9321 13.5209 11.1082 13.7192C11.2843 13.9175 11.5269 14.0443 11.7902 14.0756L11.917 14.0832V17.3332L11.9246 17.4599C11.9531 17.7017 12.0622 17.9269 12.2344 18.0991C12.4066 18.2712 12.6318 18.3804 12.8736 18.4089L13.0003 18.4165H14.0837L14.2104 18.4089C14.4522 18.3804 14.6774 18.2712 14.8496 18.0991C15.0217 17.9269 15.1309 17.7017 15.1594 17.4599L15.167 17.3332L15.1594 17.2064C15.1335 16.9854 15.0402 16.7777 14.8921 16.6116C14.744 16.4455 14.5484 16.329 14.3317 16.278L14.2104 16.2563L14.0837 16.2498V12.9998L14.0761 12.8731C14.0476 12.6313 13.9384 12.4061 13.7662 12.2339C13.5941 12.0618 13.3689 11.9526 13.1271 11.9241L13.0003 11.9165ZM13.0112 8.6665L12.8736 8.67409C12.6103 8.70541 12.3676 8.83221 12.1915 9.03047C12.0154 9.22873 11.9182 9.48468 11.9182 9.74984C11.9182 10.015 12.0154 10.2709 12.1915 10.4692C12.3676 10.6675 12.6103 10.7943 12.8736 10.8256L13.0003 10.8332L13.1379 10.8256C13.4012 10.7943 13.6439 10.6675 13.82 10.4692C13.996 10.2709 14.0933 10.015 14.0933 9.74984C14.0933 9.48468 13.996 9.22873 13.82 9.03047C13.6439 8.83221 13.4012 8.70541 13.1379 8.67409L13.0112 8.6665Z"
                                        fill="#5D87FF" />
                                </svg>
                                <p id="detail-title" class="pt-1"></p>
                            </h6>
                            <p class="px-4 ms-2 pt-0 mt-0" id="detail-description"></p>
                        </div>
                        <div class="px-4 pt-2">
                            <h6>Jawaban</h6>
                            ...
                            <input type="text" id="challengeId-edit" hidden name="challenge_id">
...
@section('script')
    ...
    <script>
        ...
        $('.btn-detail').click(function() {
            ...
            $('#form-update').attr('action', '/siswa-online/challenge/update/' + id);
