...
@section('content')
            ...
            <section id="other-course">
                ...
                <div class="row">
                    @forelse ($course->subCourses as $data)
                        ...
                        <div class="col-lg-12 m-0 p-0">
                            <a href="@if($subCourse->id != $data->id) {{ route('siswa-offline.sub-course.detail', ['subCourse' => $data->id, 'course' => $course->id]) }} @else # @endif" class="card card-body mb-3 border-rounded @if($subCourse->id == $data->id) border-2 bg-light-primary border-primary @endif"  style="border-left: 6px solid var(--bs-gray-300); border-left-color: var(--bs-primary);">
    ...
    <div class="card-floating card card-body">
        <a href="{{($prev != null) ? route('siswa-offline.sub-course.detail', ['course' => $course->id, 'subCourse' => $prev->id]) : '#' }}" class="navigation-next @if ($prev == null) disabled @endif">
        ...
        <a href="{{($next != null) ? route('siswa-offline.sub-course.detail', ['course' => $course->id, 'subCourse' => $next->id]) : '#' }}" class="navigation-next @if ($next == null) disabled @endif">
