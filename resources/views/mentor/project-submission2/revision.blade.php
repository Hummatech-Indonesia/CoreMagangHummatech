...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between w-100 mb-4 gap-2 navbar-shadow">
            <a class="text-decoration-none" href="/mentor/project-submissions/{{$project->id}}/detail">
                    ...
                    @foreach ($revisions->presentation->revision as $revision)
                        <tr>
                            ...
                            <td>
                                <span
                                @class([
    'bg-label-danger text-sm p-2 rounded-pill' => $revision->status == \App\Enum\RevisionStatusEnum::Todo->value,
    'bg-label-warning text-sm p-2 rounded-pill' => $revision->status == \App\Enum\RevisionStatusEnum::InProgress->value,
    'bg-label-primary text-sm p-2 rounded-pill' => $revision->status == \App\Enum\RevisionStatusEnum::Completed->value
])
                                >
